...
contract TroveManager is TroveManagerBase, ITroveManager, ReentrancyGuard {
    ...
    // IUSMToken internal yusdTokenContract;

    // IMOJOToken internal yetiTokenContract;

    // ISMOJO internal sYETIContract;
    ...
    //     yusdTokenContract = IUSMToken(_yusdTokenAddress);
    //     sortedTroves = ISortedTroves(_sortedTrovesAddress);
    //     yetiTokenContract = IMOJOToken(_yetiTokenAddress);
    //     sYETIContract = ISMOJO(_sYETIAddress);
    ...
    function _updateTrove(
        ...
        uint256 ICR = _getICRColls(troveColl, debt);
        ...
        returns (uint256)
    {
        ...
        uint256 ICR = _getICRColls(colls, currentYUSDDebt);
        return ICR;
    }
        ...
        returns (uint256 pendingYUSDDebtReward)
    {
        ...
        for (uint256 i; i < allCollsLen; ++i) {
            address coll = allColls[i];
            uint256 snapshotYUSDDebt = rewardSnapshots[_borrower].YUSDDebts[
                coll
            ];
            ...
            pendingYUSDDebtReward = pendingYUSDDebtReward.add(
                assetYUSDDebtReward
            );
        }
    }
        ...
        for (uint256 i; i < tokensLen; ++i) {
            ...
            if (totalStakes[token] != 0) {
                ...
                lastYUSDDebtError_Redistribution[token] = YUSDDebtNumerator.sub(
                    YUSDDebtRewardPerUnitStaked.mul(
                        thisTotalStakes.mul(10**(18 - dec))
                    )
                );
                ...
                L_YUSDDebt[token] = L_YUSDDebt[token].add(
                    YUSDDebtRewardPerUnitStaked
                );
    ...
    function updateSystemSnapshots_excludeCollRemainder(
        ...
        for (uint256 i; i < tokensLen; ++i) {
            ...
            totalCollateralSnapshot[token] = activeColl
                .sub(_tokenRemainder)
                .add(liquidatedColl);
        }
    ...
    function getL_YUSD(address _token)
        external
        view
        override
        returns (uint256)
    ...
    function yusdToken() external view override returns (IUSMToken) {
        return yusdTokenContract;
    }

    function yetiToken() external view override returns (IMOJOToken) {
        return yetiTokenContract;
    }

    function sYETI() external view override returns (ISMOJO) {
